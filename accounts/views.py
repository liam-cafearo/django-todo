...
class UserView(APIView):
    ...
    def post(self, request):
        ...
        Returns a 201 (succesfully created) if the user is successfully
        created, otherwise returns a 400 (bad request)
        """
        serializer = UserSerializer(data=request.data)

        # Check to see if the data in the `request` is valid
        # If the data cannot be deserialized into a user object then
        ...
        if not serializer.is_valid():
            ...
            # using the password provided the `data` dict
